...
dfXge = pd.read_csv('results/APA/xge_articles.csv')
...
xgeCleaned.to_csv("results/APA/xge_articles_cleaned.csv", index=False)
# Write xgeCleaned to a JSON file using json.dump
with open("results/APA/xge_articles_cleaned.json", "w", encoding="utf-8") as json_file:
    json.dump(xgeCleaned.to_dict(orient="records"), json_file, indent=4, ensure_ascii=False)


####### Psychological Science ########
# Read in the CSV file
dfPss = pd.read_csv('results/Sage/pss_articles.csv')
...
pssCleaned.to_csv("results/Sage/pss_articles_cleaned.csv", index=False)
# Write pssCleaned to a JSON file using json.dump
with open("results/Sage/pss_articles_cleaned.json", "w", encoding="utf-8") as json_file:
    json.dump(pssCleaned.to_dict(orient="records"), json_file, indent=4, ensure_ascii=False)


# Psychonomic Bulletin and Review
# Read in the CSV file
dfPbr = pd.read_csv('results/Springer/pbr_articles.csv')
pbrCleaned = dfPbr
...
pbrCleaned.to_csv("results/Springer/pbr_articles_cleaned.csv", index=False)
# Write pbrCleaned to a JSON file using json.dump
with open("results/Springer/pbr_articles_cleaned.json", "w", encoding="utf-8") as json_file:
    json.dump(pbrCleaned.to_dict(orient="records"), json_file, indent=4, ensure_ascii=False)


# Developmental Science
# Read in the CSV file
dfDs = pd.read_csv('results/Wiley/ds_articles.csv')
dsCleaned = dfDs
...
# Write pbrCleaned to a CSV file
dsCleaned.to_csv("results/Wiley/ds_articles_cleaned.csv", index=False)
# Write pbrCleaned to a JSON file using json.dump
with open("results/Wiley/ds_articles_cleaned.json", "w", encoding="utf-8") as json_file:
